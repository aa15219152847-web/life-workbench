 ...
 *
 * 用法:
 *   node issue-license.js 12            # 生成 1 个 12 个月兑换码（默认）
 *   node issue-license.js 3 10          # 生成 10 个 3 个月兑换码
...
function listLedger() {
  ...
  console.log('状态\t时长\t批次\t\t发码时间\t\t兑换码');
  for (const it of list) {
    const expired = it.exp < now ? '已过期' : '有效';
    console.log(`${expired}\t${it.months}个月\t${(it.batch || '-').slice(0, 8)}\t${new Date(it.issue).toLocaleString('zh-CN')}\t${it.code}`);
  }
}
...
for (let i = 0; i < count; i++) {
  const code = makeCode(months);
  codes.push(code);
  ledger.push({ code, months, batch, issue: Date.now(), exp: Date.now() + months * 30 * 24 * 3600 * 1000 });
}
saveLedger(ledger);
